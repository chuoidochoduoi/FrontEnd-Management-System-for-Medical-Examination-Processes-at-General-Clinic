...
export default function AppointmentConfirmModal({ data, onClose, onConfirm, namespace = 'appointment' }) {
    ...
    const patientRows = [
        ...
        { key: tConfirm('email'),      val: data.email },
        { key: tConfirm('address'),    val: data.address },
    ];

    const timeRows = [
        { key: tConfirm('date'),      val: data.date,     bold: true },
        { key: tConfirm('timeSlot'),  val: data.timeSlot },
        { key: tConfirm('method'),    val: data.method },
    ];
        ...
        <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-50 px-4">
            <div className="bg-white rounded-xl w-full max-w-[560px] overflow-hidden">
                ...
                <div className="px-6 pt-5 pb-4 border-b border-gray-100 relative">
                    <h2 className="text-[17px] font-medium text-gray-900">{tConfirm('title')}</h2>
                    <p className="text-[13px] text-gray-500 mt-1">{tConfirm('subtitle')}</p>
                ...
                <div className="px-6 py-5 grid grid-cols-2 gap-4">
                    ...
                    {/* Bệnh nhân */}
                    <div>
                        <p className="text-xs text-gray-400 mb-2">{tConfirm('patientInfo')}</p>
                    ...
                    {/* Thời gian */}
                    <div>
                        <p className="text-xs text-gray-400 mb-2">{tConfirm('timeInfo')}</p>
                        <div className="border border-gray-100 rounded-lg overflow-hidden">
                            {timeRows.map((row, i) => (
                            ...
                            <div className="flex justify-between items-center px-3 py-2.5">
                                <span className="text-[13px] font-medium text-gray-900">{tConfirm('total')}</span>
                    ...
                    <div className="col-span-2">
                        <p className="text-xs text-gray-400 mb-2">{tConfirm('services')}</p>
                        <div className="border border-gray-100 rounded-lg overflow-hidden">
                            <div className="flex justify-between px-3 py-2.5 border-b border-gray-100">
                                <span className="text-[13px] text-gray-400">{tConfirm('serviceList')}</span>
                                <span className="text-[13px] text-gray-400">
                  {tConfirm('serviceCount', { count: data.services.length.toString().padStart(2, '0') })}
                </span>
                            </div>
                            ...
                            <div className="flex gap-4 px-3 py-2.5">
                                <span className="text-[13px] text-gray-400 shrink-0">{tConfirm('reason')}</span>
