...
const SidebarItem = ({ icon: Icon, label, active = false }) => (
  <button className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-sm transition-all ${
    active ? 'bg-[#1ab2a6] text-white' : 'text-slate-500 hover:bg-slate-50'
  }`}>
    <Icon size={20} />
    {label}
  </button>
...
const PrescriptionItem = ({ name, type, dosage, qty }) => (
        ...
        <div className="p-8 max-w-[1600px] mx-auto">
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 xl:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-[32px] p-6 border-2 border-red-500/20 shadow-sm relative">
                <div className="flex items-center justify-between mb-6">
                ...
                <div className="relative mb-6">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-10 pr-4 py-2.5 text-xs outline-none focus:ring-2 focus:ring-blue-500/10 transition-all"
                  />
                </div>

                <div className="space-y-4">
                  {prescriptions.map((pill, idx) => (
                    <PrescriptionItem key={idx} {...pill} />
                  ))}
                </div>

                <button className="w-full mt-6 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-slate-800 active:scale-[0.98] transition-all shadow-lg">
                  Manage Medications
                </button>
              </div>
            </div>
