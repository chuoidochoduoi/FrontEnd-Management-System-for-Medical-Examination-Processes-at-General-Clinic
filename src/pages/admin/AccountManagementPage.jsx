...
import { Search, X, Upload, User } from "lucide-react";
      ...
      >
        <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-xl">
          ...
          <div className="px-7 py-6 space-y-6">
            {/* ── KHỐI UPLOAD ẢNH (Không có nút Xóa ảnh cho Admin) ── */}
            <div className="p-3.5 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
              <label className="text-xs font-semibold text-gray-600 block">
                Ảnh đại diện nhân sự (không bắt buộc)
              </label>
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-white border border-gray-300 shadow-xs shrink-0 flex items-center justify-center overflow-hidden">
                  {form.avatarUrl ? (
                      <img
                          src={form.avatarUrl}
                          alt="Avatar Preview"
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-xs font-semibold text-gray-700 hover:bg-gray-100 shadow-xs cursor-pointer transition"
                    >
                      <Upload size={13} /> Chọn ảnh từ máy tính
                    </button>
                  </div>
                  <input
                      type="text"
                      placeholder="Hoặc dán link ảnh trực tuyến (https://...)..."
                      value={form.avatarUrl?.startsWith('data:image') ? 'Đã tải ảnh lên từ máy tính' : form.avatarUrl}
                      disabled={form.avatarUrl?.startsWith('data:image')}
                      onChange={(e) => set("avatarUrl", e.target.value)}
                      className="w-full text-xs px-2.5 py-1.5 border border-gray-200 rounded-lg outline-none bg-white focus:border-primary-500"
                  />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-5">
                {[
                  { key: "username", label: t("accountManagement.addStaffModal.fields.loginAccount") },
                  { key: "password", label: t("accountManagement.addStaffModal.fields.password"), type: "password" },
                  { key: "systemRole", label: t("accountManagement.addStaffModal.fields.systemTitle"), type: "select" },
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 border border-gray-200 rounded-lg p-3 max-h-40 overflow-y-auto">
                      {capabilities.map(capability => (
                          <label key={capability.capabilityId} className="flex items-center gap-2 text-sm">
                            <input
                                type="checkbox"
                                checked={form.capabilityIds.includes(capability.capabilityId)}
                                onChange={event => set("capabilityIds", event.target.checked ? [...form.capabilityIds, capability.capabilityId] : form.capabilityIds.filter(id => id !== capability.capabilityId))}
                            />
                            {capability.name}
                          </label>
                      ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-5">
                {[
                  ...
                  { key: "phone", label: t("accountManagement.addStaffModal.fields.phone"), type: "tel" },
                  { key: "email", label: t("accountManagement.addStaffModal.fields.email"), type: "email" },
...
function UpdateStaffModal({ account, onClose, staffHook, t }) {
  const { specializations } = useSpecializations();
  const { capabilities } = useCapabilities();
  const fileInputRef = useRef(null);
  ...
  const inputCls =
      "w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2.5 outline-none text-sm text-gray-900 caret-gray-900 placeholder:text-gray-400 focus:border-primary-500 focus:bg-white focus:ring-2 focus:ring-primary-100 transition-colors";
  const labelCls = "block text-xs font-semibold text-gray-500 tracking-wide mb-1.5";
                ...
                <div className="px-4 py-3 border-b border-gray-100 flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                        ...
                        placeholder={t("accountManagement.patients.searchPlaceholder")}
                        className="w-full h-10 pl-9 pr-4 text-sm outline-none bg-transparent placeholder:text-gray-300"
                    />
                  </div>
                  <select
