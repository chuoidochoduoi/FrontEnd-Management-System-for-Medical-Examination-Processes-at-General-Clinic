...
const readJson = async response => { const data = await response.json().catch(() => null); if (!response.ok) throw new Error(data?.message || data?.error || 'Yêu cầu không thành công'); return data?.data ?? data; };
const exceptionTypeLabel = type => ({ CLOSED_DAY: 'Đóng cửa cả ngày', SHIFT_OFF: 'Nghỉ một ca', SPECIAL_HOURS: 'Giờ hoạt động đặc biệt' }[type] || type);
const coverageReasonLabel = reason => ({
    CLINIC_CLOSED: 'Phòng khám đóng cửa', SHIFT_OFF: 'Ca này nghỉ', NO_ACTIVE_SHIFT_VERSION: 'Ca chưa có khung giờ hiệu lực',
    SERVICE_INACTIVE: 'Dịch vụ đã ngừng áp dụng', DEPARTMENT_UNAVAILABLE: 'Phòng thực hiện không khả dụng',
    CAPABILITY_UNAVAILABLE: 'Chưa có phòng đủ năng lực kỹ thuật', NO_QUALIFIED_STAFF: 'Chưa có nhân sự đủ điều kiện',
}[reason] || reason || 'Chưa đáp ứng điều kiện phục vụ');
...
export default function ShiftManagementPage() {
    ...
    return <AdminLayout><div className="max-w-7xl mx-auto space-y-6">
        ...
        : <section className="grid xl:grid-cols-[1.4fr_1fr] gap-6"><div className="bg-white border border-slate-200 rounded-2xl p-5"><div className="flex justify-between items-center mb-5"><div><h2 className="font-bold">Lịch theo tháng</h2><p className="text-xs text-slate-500">Chọn một ngày trong tương lai để cấu hình</p></div><input type="month" value={month} onChange={e => setMonth(e.target.value)} className="border rounded-lg px-3 py-2 text-sm"/></div><div className="grid grid-cols-7 text-center text-xs font-bold text-slate-400 mb-2">{['Thứ 2','Thứ 3','Thứ 4','Thứ 5','Thứ 6','Thứ 7','Chủ nhật'].map(day => <span key={day}>{day}</span>)}</div><div className="grid grid-cols-7 gap-2">{Array.from({ length: monthRange.startDay }).map((_, i) => <div key={`blank-${i}`}/>)}{Array.from({ length: monthRange.days }, (_, i) => { const date = `${month}-${String(i + 1).padStart(2, '0')}`; const dayItems = exceptions.filter(item => item.workDate === date); return <button key={date} onClick={() => openDay(date)} className={`min-h-24 rounded-xl border p-2 text-left ${date < tomorrow() ? 'bg-slate-50 text-slate-300' : 'hover:border-primary-400'} ${dayItems.some(e => e.type === 'CLOSED_DAY') ? 'bg-red-50 border-red-200' : ''}`}><span className="text-sm font-bold">{i + 1}</span>{dayItems.map(e => <span key={e.exceptionId} className="block mt-1 px-1.5 py-1 rounded bg-white/80 text-[10px] truncate">{e.type === 'CLOSED_DAY' ? 'Đóng cửa' : `${e.shiftName}: ${e.type === 'SHIFT_OFF' ? 'Nghỉ ca' : `${e.specialStartTime?.slice(0,5)}–${e.specialEndTime?.slice(0,5)}`}`}</span>)}</button>; })}</div></div>
            <div className="space-y-5"><div className="bg-white border border-slate-200 rounded-2xl p-5"><h2 className="font-bold flex items-center gap-2"><ShieldAlert size={18}/>Kiểm tra khả năng phục vụ</h2><div className="grid grid-cols-2 gap-3 mt-4"><input type="date" min={tomorrow()} value={coverageDate} onChange={e => setCoverageDate(e.target.value)} className="border rounded-lg px-3 py-2 text-sm"/><select value={coverageShift} onChange={e => setCoverageShift(e.target.value)} className="border rounded-lg px-3 py-2 text-sm"><option value="">Chọn ca làm việc</option>{shifts.filter(s => s.isActive).map(s => <option key={s.shiftId} value={s.shiftId}>{s.name}</option>)}</select></div><button onClick={loadCoverage} className="mt-3 w-full rounded-lg bg-slate-900 text-white py-2 text-sm font-semibold">Kiểm tra khả năng phục vụ</button>{coverage && <div className="mt-4"><div className="grid grid-cols-3 gap-2 text-center"><Metric label="Dịch vụ" value={coverage.totalActiveServices}/><Metric label="Đáp ứng" value={coverage.coveredServices} good/><Metric label="Chưa đáp ứng" value={coverage.uncoveredServices} bad/></div><div className="mt-3 max-h-64 overflow-auto divide-y">{coverage.services.filter(s => !s.available).map(s => <div key={s.serviceId} className="py-2"><p className="text-xs font-semibold">{s.serviceCode} — {s.serviceName}</p><p className="text-[11px] text-red-600">{coverageReasonLabel(s.reason)}</p></div>)}</div></div>}</div><div className="bg-white border border-slate-200 rounded-2xl p-5"><h2 className="font-bold">Lịch ngoại lệ đã cấu hình</h2><div className="mt-3 max-h-72 overflow-auto divide-y">{exceptions.length === 0 ? <p className="text-sm text-slate-400 py-4">Tháng này chưa có lịch ngoại lệ</p> : exceptions.map(item => <div key={item.exceptionId} className="py-3 flex justify-between gap-3"><div><p className="text-sm font-semibold">{item.workDate} · {exceptionTypeLabel(item.type)}</p><p className="text-xs text-slate-500">{item.shiftName || 'Toàn phòng khám'} — {item.reason}</p></div><button onClick={() => reopen(item.exceptionId)} className="text-xs text-primary-600 font-semibold">Mở lại</button></div>)}</div></div></div></section>}
    </div>
...
function ExceptionForm({ value, setValue, shifts, impact, preview, save, saving }) { const requiresShift = value.type !== 'CLOSED_DAY'; const special = value.type === 'SPECIAL_HOURS'; const valid = value.reason && (!requiresShift || value.shiftId) && (!special || value.specialStartTime && value.specialEndTime); return <div className="space-y-4"><Field label="Hình thức áp dụng"><select className={inputClass} value={value.type} onChange={e => setValue({ ...value, type: e.target.value, shiftId: e.target.value === 'CLOSED_DAY' ? null : '', specialStartTime: null, specialEndTime: null })}><option value="CLOSED_DAY">Đóng cửa toàn phòng khám</option><option value="SHIFT_OFF">Cho nghỉ một ca</option><option value="SPECIAL_HOURS">Điều chỉnh giờ của một ca</option></select></Field>{requiresShift && <Field label="Ca làm việc"><select className={inputClass} value={value.shiftId || ''} onChange={e => setValue({ ...value, shiftId: e.target.value })}><option value="">Chọn ca làm việc</option>{shifts.filter(s => s.isActive).map(s => <option key={s.shiftId} value={s.shiftId}>{s.name}</option>)}</select></Field>}{special && <div className="grid grid-cols-2 gap-3"><TimeField label="Giờ bắt đầu đặc biệt" value={value.specialStartTime || ''} onChange={specialStartTime => setValue({ ...value, specialStartTime })}/><TimeField label="Giờ kết thúc đặc biệt" value={value.specialEndTime || ''} onChange={specialEndTime => setValue({ ...value, specialEndTime })}/></div>}<Field label="Lý do"><textarea className={inputClass} rows="3" value={value.reason} onChange={e => setValue({ ...value, reason: e.target.value })}/></Field><button disabled={!valid} onClick={preview} className="w-full border rounded-xl py-2.5 text-sm font-semibold disabled:opacity-40">Xem trước ảnh hưởng</button>{impact && <Impact value={impact}/>}<SaveButton onClick={save} disabled={saving || !valid || !impact || impact.blocked}/></div>; }
