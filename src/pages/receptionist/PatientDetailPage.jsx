...
export default function PatientDetailPage() {
    ...
    return <ReceptionistLayout><div className="mx-auto max-w-6xl space-y-6">
        ...
        {patient && <>
            <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm"><h2 className="mb-5 text-sm font-bold text-gray-900">Thông tin cá nhân</h2><div className="grid grid-cols-2 gap-x-10 gap-y-5 lg:grid-cols-4">
                {[['Mã bệnh nhân', patient.patientCode], ['Họ và tên', patient.fullName], ['Số điện thoại', patient.phone], ['Email', patient.email], ['Ngày sinh', patient.dateOfBirth ? new Date(patient.dateOfBirth).toLocaleDateString('vi-VN') : '-'], ['Giới tính', patient.gender === 'MALE' ? 'Nam' : patient.gender === 'FEMALE' ? 'Nữ' : '-'], ['Nhóm máu', patient.bloodType], ['Địa chỉ', patient.address]].map(([label, content]) => <div key={label}><p className="text-xs text-gray-400">{label}</p><p className="mt-1 text-sm font-medium text-gray-800">{value(content)}</p></div>)}
            </div></section>
