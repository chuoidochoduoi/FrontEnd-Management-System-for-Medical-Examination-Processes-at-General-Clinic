...
export default function PatientVisitDetailPage() {
    ...
    useEffect(() => { fetch(`${import.meta.env.VITE_API_URL}/api/receptionist/customers/${id}/visits/${visitId}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token') || sessionStorage.getItem('token')}` } }).then(async res => { if (!res.ok) throw new Error('Không thể tải bệnh án.'); return res.json(); }).then(setVisit).catch(e => setError(e.message)); }, [id, visitId]);
