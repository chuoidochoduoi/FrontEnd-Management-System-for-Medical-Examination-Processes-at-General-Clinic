...
import { UserCircle, Key, Phone, Mail, User, Shield, Briefcase, MapPin, Calendar, Edit2, Check, X, Users, Camera, Trash2 } from 'lucide-react';
...
export default function StaffProfilePage() {
            ...
            try {
                ...
                if (staffRes.ok) {
                    ...
                    setEditForm({
                        ...
                        avatarUrl: staffData.profile?.avatarUrl || '',
    ...
    const handleAvatarFileChange = (e) => {
        ...
        if (!file.type.startsWith("image/")) {
            toast.error("Vui lòng chọn đúng file hình ảnh (JPG, PNG, JPEG).");
            return;
        }
        ...
        reader.onloadend = () => {
            setEditForm(p => ({ ...p, avatarUrl: reader.result }));
            toast.success("Đã chọn ảnh đại diện mới. Bấm 'Lưu hồ sơ' để hoàn tất!");
        };
        ...
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/profiles/me`, {
                ...
                body: JSON.stringify({
                    ...
                    // Truyền chuỗi rỗng "" hoặc giá trị chuẩn để backend xử lý xóa dứt điểm
                    avatarUrl: editForm.avatarUrl ? editForm.avatarUrl.trim() : ""
                })
            });
            if (!res.ok) {
    ...
    const renderContent = () => {
                ...
                <div className="max-w-2xl mx-auto">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
                        ...
                        <div className="relative">
                            <div className="w-28 h-28 rounded-full bg-gradient-to-br from-primary-100 to-primary-50 flex items-center justify-center shadow-inner overflow-hidden">
        ...
        const currentAvatar = editing ? editForm?.avatarUrl : profile?.profile?.avatarUrl;
        ...
        return (
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col md:flex-row items-center md:items-start gap-8 relative overflow-hidden">
                    ...
                    {/* KHUNG TRÒN AVATAR NHẤN VÀO ĐỂ TẢI ẢNH VÀ NÚT XÓA ẢNH */}
                    <div className="flex flex-col items-center gap-3 shrink-0">
                        <div className="relative group">
                            <div
                                onClick={() => editing && fileInputRef.current?.click()}
                                className={`w-32 h-32 rounded-full bg-gradient-to-br from-primary-100 to-primary-50 flex items-center justify-center shadow-md overflow-hidden border-4 border-white transition-all duration-200 ${
                                    editing ? 'cursor-pointer hover:ring-4 hover:ring-primary-200 group-hover:opacity-90' : ''
                                }`}
                                title={editing ? "Nhấn vào đây để tải ảnh đại diện lên" : ""}
                            >
                                {currentAvatar ? (
                                    <img
                                        src={currentAvatar}
                                ...
                                {editing && (
                                    <div className="absolute inset-0 bg-black/40 rounded-full flex flex-col items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <span className="text-[10px] font-semibold tracking-wider uppercase">Đổi ảnh</span>
                                    </div>
                                )}
                            </div>
                            ...
                            {editing && (
                                <button
                                    ...
                                    className="absolute bottom-0 right-0 p-2 bg-primary-600 hover:bg-primary-700 text-white rounded-full shadow-lg border-2 border-white transition-transform active:scale-90"
                        ...
                        {/* Nút xóa ảnh hiển thị khi đang ở chế độ chỉnh sửa và đã có ảnh */}
                        {editing && editForm?.avatarUrl && (
                            <button
                                type="button"
                                onClick={() => setEditForm(p => ({ ...p, avatarUrl: "" }))}
                                className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-xs font-semibold transition border border-red-200 shadow-xs cursor-pointer"
                            >
